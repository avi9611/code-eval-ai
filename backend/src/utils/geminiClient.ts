...
export async function evaluateCodeWithGemini(taskName: string, codeSnippet: string) {
  try {
...
export async function evaluateScreenshotWithGemini(taskName: string, imageBuffer: Buffer) {
  try {
    const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });
